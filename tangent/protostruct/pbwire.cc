...
template <typename T>
int _parse_delmited(pbwire_ParseContext ctx, T* value_out, size_t* value_len) {
  uint32_t length = 0;
...
int pbwire_parse_message(pbwire_ParseContext ctx,
  ...
  while (ctx.buffer.ptr != ctx.buffer.end) {
    ...
    pbwire_ParseContext sub_ctx = ctx;
    ...
    bytes_read = fielditem_callback(ctx, userdata, tag);
    if (bytes_read < 0) {
      return bytes_read;
    }
    ctx.buffer.ptr += bytes_read;
  }
  return (ctx.buffer.end - ctx.buffer.begin);
}
...
int pbwire_parse_packed_repeated(pbwire_ParseContext ctx,
  ...
  char* write_ptr = (char*)userdata;

  bytes_read = pbwire_parse_varint32(ctx, &bytes_packed);
  if (bytes_read < 0) {
    return bytes_read;
  }
  ctx.buffer.begin += bytes_read;
  ctx.buffer.ptr = ctx.buffer.begin;
  ctx.buffer.end = ctx.buffer.begin + bytes_packed;

  for (*write_idx = 0; *write_idx < array_size; (*write_idx)++) {
    ctx.buffer.ptr += bytes_read;

    if (ctx.buffer.ptr >= ctx.buffer.end) {
      return (int)bytes_packed;
    }
...
int pbwire_emitcontext_init(pbwire_EmitContext* ctx, int length_cache_size,
                            int buffer_size, pbwire_Error* error) {
  memset(ctx, 0, sizeof(pbwire_EmitContext));
  ctx->length_cache.begin =
      reinterpret_cast<uint32_t*>(calloc(length_cache_size, sizeof(uint32_t)));
  ctx->length_cache.ptr = ctx->length_cache.begin;
  ctx->length_cache.end = ctx->length_cache.begin + length_cache_size;

  ctx->buffer.begin =
      reinterpret_cast<char*>(calloc(buffer_size, sizeof(char)));
  ctx->buffer.ptr = ctx->buffer.begin;
  ctx->buffer.end = ctx->buffer.begin + buffer_size;

  ctx->error = error;
  return 0;
}

int pbwire_emitcontext_cleanup(pbwire_EmitContext* ctx) {
  free(ctx->buffer.begin);
  free(ctx->length_cache.begin);
  memset(ctx, 0, sizeof(pbwire_EmitContext));
  return 0;
}
...
int pbparse_bytes(pbwire_ParseContext ctx, uint8_t* value, size_t* value_len) {
  return _parse_delmited(ctx, value, value_len);
}
